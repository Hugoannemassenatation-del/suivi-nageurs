...
export default function ComptesAdmin({ swimmers, reload }) {
  ...
  const [swimForm, setSwimForm] = useState({ prenom: "", nomFamille: "", groupe: GROUPES[0], naissance: "", sexe: "", email: "" });
  ...
  const createSwimmer = async () => {
    setMsg(""); setSending(true);
    if (!swimForm.prenom.trim() || !swimForm.nomFamille.trim()) {
      setMsg("Prénom et nom sont obligatoires."); setSending(false); return;
    }
    const nom = `${swimForm.prenom.trim()} ${swimForm.nomFamille.trim()}`;
    const { error } = await supabase
      .from("swimmers")
      .insert({ prenom: swimForm.prenom.trim(), nom_famille: swimForm.nomFamille.trim(), nom, groupe: swimForm.groupe, naissance: swimForm.naissance, sexe: swimForm.sexe || null, email: swimForm.email.trim() || null });
    if (error) { setMsg("Erreur lors de la création du nageur : " + error.message); setSending(false); return; }
    setMsg(`"${nom}" créé. Quand vous êtes prêt(e), envoyez l'invitation depuis le panneau "Envoyer une invitation" plus bas.`);
    setSwimForm({ prenom: "", nomFamille: "", groupe: GROUPES[0], naissance: "", sexe: "", email: "" });
    setSending(false);
    reload();
  };
  ...
  const prepareInviteFor = (swimmer) => {
    setExtraForm({ swimmerId: swimmer.id, nom: "", familleRole: "parent", email: swimmer.email || "" });
    document.getElementById("panel-envoyer-invitation")?.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  ...
  return (
    <div className="view">
      ...
      <div className="panel note">
        <ShieldCheck size={15} />
        <span>Vous seul(e) pouvez créer des comptes coach et des fiches nageur. La création d'un nageur n'envoie aucun email — vous décidez du moment où envoyer l'invitation, depuis le panneau "Envoyer une invitation" ou le bouton "Inviter" à côté de chaque nageur.</span>
      </div>
      ...
      <div className="panel">
        <div className="panel-head"><h4>Créer un nageur</h4></div>
        <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>Aucun email n'est envoyé à cette étape — paramétrez tranquillement, vous inviterez la famille plus tard.</p>
        <div className="form-row">
        ...
        <div className="form-row">
          <input placeholder="Email (optionnel — pourra être ajouté plus tard)" type="email" value={swimForm.email} onChange={(e) => setSwimForm({ ...swimForm, email: e.target.value })} />
          <button className="btn-primary" disabled={sending} onClick={createSwimmer}><Plus size={14} /> {sending ? "Création…" : "Créer le nageur"}</button>
        </div>
      </div>

      <div className="panel" id="panel-envoyer-invitation">
        <div className="panel-head"><h4>Envoyer une invitation (parent ou nageur)</h4></div>
        <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>Sélectionnez un nageur déjà créé et envoyez l'accès quand vous êtes prêt(e) — au parent et/ou au nageur, chacun avec son propre email.</p>
        <div className="form-row">
          <select value={extraForm.swimmerId} onChange={(e) => {
            const sw = swimmers.find((s) => s.id === e.target.value);
            setExtraForm({ ...extraForm, swimmerId: e.target.value, email: extraForm.email || sw?.email || "" });
          }}>
        ...
        <div className="form-row">
          ...
          <button className="btn-primary" disabled={sending} onClick={createExtraFamilyInvite}><Plus size={14} /> {sending ? "Envoi…" : "Envoyer l'invitation"}</button>
        </div>
      </div>
      ...
      <div className="panel">
        ...
        {swimmers.length === 0 ? <EmptyState text="Aucun nageur créé." /> : (
          swimmers.map((s) => (
            <div key={s.id} className="row-line" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>{s.nom} — {s.groupe}{s.email ? ` — ${s.email}` : ""}</span>
              <span className="table-actions">
                <button className="btn-secondary" onClick={() => prepareInviteFor(s)}>Inviter</button>
