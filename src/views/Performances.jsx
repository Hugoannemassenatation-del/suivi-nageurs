...
import { EPREUVES, BASSINS, todayISO, formatSeconds, parseTimeToSeconds, isOlderYear } from "../lib/format";
...
const epreuveIndex = (epreuve) => {
  const i = EPREUVES.indexOf(epreuve);
  return i === -1 ? 999 : i;
};
...
export default function Performances({ swimmers, performances, splits, readOnly, reload }) {
  ...
  const importBulk = async () => {
    ...
    lines.forEach((line, i) => {
      ...
      if (!swimmer || !epreuve || !bassin || !date || secs == null) { ignorees.push(i + 1); return; }
      rows.push({ swimmer_id: swimmer.id, epreuve, bassin, date, secondes: secs });
    });
  ...
  const chartData = useMemo(() => {
    ...
    return performances
      .filter((p) => p.swimmer_id === selSwimmer && p.epreuve === selEpreuve && p.bassin === selBassin)
...
function SwimmerFiche({ swimmer, performances, bestBySwimmer, onBack, onOpenDetail, onRemove, readOnly }) {
  ...
  const groupes = ["25m", "50m"].map((bassin) => {
    ...
    const parEpreuve = {};
    perfsDuBassin.forEach((p) => { (parEpreuve[p.epreuve] ||= []).push(p); });
