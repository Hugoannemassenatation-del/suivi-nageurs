...
import { Plus, Trash2, TrendingUp, ClipboardEdit, ArrowLeft, Upload, AlertTriangle, ChevronDown, ChevronRight } from "lucide-react";
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";
import { ViewHeader, EmptyState, Modal } from "../lib/ui";
import { EPREUVES, BASSINS, todayISO, formatSeconds, parseTimeToSeconds, isOlderYear, normalizeEpreuve, formatDateFR } from "../lib/format";
import { createPerformance, updatePerformance, deletePerformance, deleteAllPerformances, createPerformanceSplit, deletePerformanceSplit, createPerformancesBulk } from "../lib/db";
...
export default function Performances({ swimmers, performances, splits, readOnly, reload }) {
  const [detailIdFamille, setDetailIdFamille] = useState(null);

  if (readOnly) {
    return (
      <>
        <FamillePerformances performances={performances} onOpenDetail={setDetailIdFamille} />
        {detailIdFamille && (
          <DetailModal
            performance={performances.find((p) => p.id === detailIdFamille)}
            splits={splits.filter((s) => s.performance_id === detailIdFamille)}
            nom={swimmers[0]?.nom || ""}
            readOnly={true}
            onClose={() => setDetailIdFamille(null)}
            reload={reload}
          />
        )}
      </>
  ...
  return <PerformancesStaff swimmers={swimmers} performances={performances} splits={splits} reload={reload} />;
}

function FamillePerformances({ performances, onOpenDetail }) {
  ...
  return (
    <div className="view">
      <ViewHeader eyebrow="Mes performances" title="Performances" />
      {groupes.length === 0 ? <EmptyState text="Aucun chrono enregistré pour l'instant." /> : (
        groupes.map((g) => (
          <div className="panel" key={g.bassin}>
            ...
            {g.epreuves.map((epreuve) => {
              ...
              const best = list.reduce((m, p) => (p.secondes < m.secondes ? p : m), list[0]);
              const key = g.bassin + "|" + epreuve;
              const isOpen = !!expanded[key];
              return (
                <div key={epreuve} style={{ marginBottom: 8 }}>
                  <button className="session-item" style={{ width: "100%", cursor: "pointer", textAlign: "left", font: "inherit" }} onClick={() => toggle(key)}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {isOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                      <div>
                        <div className="session-title">{epreuve}</div>
                        <div className="session-meta">{list.length} chrono{list.length > 1 ? "s" : ""}</div>
                      </div>
                    </div>
                    <span className="chrono-best" style={{ fontSize: 18 }}>{formatSeconds(best.secondes)}</span>
                  </button>
                  {isOpen && (
                    <div className="table" style={{ marginTop: 6 }}>
                      ...
                      {list.map((p) => (
                        <div className="table-row fiche-perf-row" key={p.id}>
                          <span className={(p.id === best.id ? "chrono-best " : "") + (isOlderYear(p.date) ? "chrono-old" : "")}>{formatSeconds(p.secondes)}{p.id === best.id && " ★"}</span>
...
function PerformancesStaff({ swimmers, performances, splits, reload, readOnly = false }) {
    ...
    <div className="view">
      ...
      {swimmers.length === 0 ? <EmptyState text="Aucun nageur." /> : (
        <>
          ...
          {!readOnly && (
            <div className="panel" style={{ borderColor: "#D64545" }}>
              <div className="panel-head"><h4 style={{ color: "#D64545" }}><AlertTriangle size={15} style={{ verticalAlign: "middle", marginRight: 6 }} />Zone dangereuse</h4></div>
              <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>Supprime définitivement tous les chronos ({performances.length} au total), pour tous les nageurs, y compris les retours de course, vidéos et temps de passage associés. Cette action est irréversible.</p>
              <button className="btn-secondary" style={{ borderColor: "#D64545", color: "#D64545" }} onClick={() => setConfirmDeleteAll(true)}>Supprimer toutes les performances</button>
            </div>
          )}
      ...
      {confirmDeleteAll && (
        <ConfirmDeleteAllModal
          ...
          onConfirmed={async () => { await deleteAllPerformances(); setConfirmDeleteAll(false); reload(); }}
        />
      )}
    </div>
  );
...
function ConfirmDeleteAllModal({ count, onClose, onConfirmed }) {
  const [text, setText] = useState("");
  const [busy, setBusy] = useState(false);
  const ok = text.trim().toUpperCase() === "SUPPRIMER";
  ...
  return (
    <Modal title="Confirmer la suppression totale" onClose={onClose}>
      <p className="login-hint">
        Vous êtes sur le point de supprimer <strong>{count} performance{count > 1 ? "s" : ""}</strong> pour tous les nageurs, de façon définitive. Cette action ne peut pas être annulée.
      </p>
      <label>Tapez SUPPRIMER pour confirmer</label>
      <input value={text} onChange={(e) => setText(e.target.value)} placeholder="SUPPRIMER" />
      <button className="btn-primary full" style={{ background: "#D64545", marginTop: 10 }} disabled={!ok || busy} onClick={confirm}>
