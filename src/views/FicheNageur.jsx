...
export default function FicheNageur({ swimmer, sessions, presences, rpes, performances, vma, wellness, onClose }) {
          ...
          {myPresences.length === 0 ? <EmptyState text="Aucune présence enregistrée." /> : (
            <div className="table" style={{ marginBottom: 20 }}>
              ...
              {myPresences.slice(0, 15).map((p) => {
                ...
                const rpeTexte = daySessions
                  .map((se) => {
                    const r = rpes.find((x) => x.session_id === se.id && x.swimmer_id === swimmer.id);
                    return r ? `${se.titre}: ${r.rpe}/10` : null;
                  })
                  .filter(Boolean)
                  .join(" · ");
                return (
