...
export default function Statistiques({ swimmers, sessions, presences, jours, jourRpes }) {
  ...
  // Deux créneaux le même jour ne doivent compter qu'une seule fois pour le volume/le nombre d'entraînements.
  const rows = useMemo(() => {
    return swimmers.map((s) => {
      const datesPresent = [...new Set(presences.filter((p) => p.swimmer_id === s.id && p.present).map((p) => p.date))];
      const compute = (filterFn) => {
        const dates = datesPresent.filter(filterFn);
        return { nb: dates.length, volume: dates.reduce((sum, date) => sum + (volumeByDate[date] || 0), 0) };
      };
      return {
        swimmer: s,
  ...
  const chargeData = useMemo(() => {
    ...
    const datesPresent = [...new Set(presences.filter((p) => p.swimmer_id === selSwimmer && p.present).map((p) => p.date))];
          ...
          <div className="panel">
            ...
            {selSwimmer && (
              chargeData.every((c) => c.charge === 0) ? (
                ...
                <>
                  {acwr && (
                    <div className="panel note" style={{ marginTop: 12, background: acwr.ratio > 1.5 ? "#FDECEC" : "var(--foam)", borderColor: acwr.ratio > 1.5 ? "#D64545" : undefined }}>
