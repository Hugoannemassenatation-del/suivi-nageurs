...
import { Plus, Trash2, ClipboardList, CheckCircle2, Settings, Upload, Mail } from "lucide-react";
...
import {
  ...
  listQuestions, createQuestion, createQuestionsBulk, deleteQuestion,
  listReponses, upsertReponse, sendNotification,
} from "../lib/db";
...
export default function Formulaires({ swimmers, isStaff, mySwimmerId }) {
  ...
  const notifierFamilles = async (f) => {
    await sendNotification({ type: "formulaire", auteur: f.titre, texte: f.description || "" });
    setNotifiedIds((ids) => [...ids, f.id]);
  };
    ...
    <div className="view">
      ...
      {isStaff && (
        <div className="panel note">
          <Mail size={15} />
          <span>La création d'un formulaire n'envoie aucun email. Cliquez « Notifier les familles » sur un formulaire actif quand il est prêt (questions ajoutées) pour les prévenir.</span>
        </div>
      )}
                ...
                <div className="session-item-actions">
                  {isStaff ? (
                    <>
                      ...
                      {f.actif && (
                        notifiedIds.includes(f.id) ? (
                          <span className="login-hint" style={{ color: "#1E8A5F", alignSelf: "center" }}>Familles notifiées ✓</span>
                        ) : (
                          <button className="btn-secondary" onClick={() => notifierFamilles(f)}><Mail size={14} style={{ marginRight: 4 }} />Notifier les familles</button>
                        )
                      )}
...
function ManageQuestionsModal({ formulaire, questions, onClose, onChanged }) {
  ...
  const TYPE_VALIDES = TYPES_QUESTION.map((t) => t.v);
  const importBulk = async () => {
    ...
    lines.forEach((line, i) => {
      ...
      const [question, type, optionsStr] = parts;
      if (!question || !TYPE_VALIDES.includes(type)) { ignorees.push(i + 1); return; }
      rows.push({
        formulaire_id: formulaire.id,
        ordre: questions.length + rows.length,
        question,
        type,
        options: (type === "choix_unique" || type === "choix_multiple") && optionsStr
          ? optionsStr.split(",").map((o) => o.trim()).filter(Boolean)
          : null,
      });
    });
    if (rows.length === 0) { setBulkMsg("Aucune ligne valide détectée."); return; }
    const { error } = await createQuestionsBulk(rows);
    if (error) { setBulkMsg("Erreur : " + error.message); return; }
    setBulkMsg(`${rows.length} question(s) importée(s)${ignorees.length ? `, ligne(s) ${ignorees.join(", ")} ignorée(s)` : ""} ✓`);
    setBulkText("");
    onChanged();
  };
    ...
    <Modal title={`Questions — ${formulaire.titre}`} onClose={onClose} wide>
      ...
      <div className="panel" style={{ marginBottom: 16 }}>
        ...
        <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>
          Une ligne par question, séparée par des points-virgules :<br />
          <code>Question;Type;Options (si choix, séparées par des virgules)</code><br />
          Types possibles : <code>texte_court</code>, <code>texte_long</code>, <code>choix_unique</code>, <code>choix_multiple</code><br />
          Exemple : <code>Es-tu disponible les week-ends ?;choix_unique;Oui,Non,Selon les dates</code>
        </p>
        <textarea rows={6} value={bulkText} onChange={(e) => setBulkText(e.target.value)} placeholder={"Quel est ton objectif chrono principal cette saison ?;texte_long;\nCombien d'entraînements par semaine t'engages-tu à suivre ?;choix_unique;1,2,3,4,5,6 ou plus"} />
        ...
        {swimmers.map((s) => {
          ...
          return (
            <div className="table-row comptes-swim-row" key={s.id}>
              ...
              <span>{r ? formatDateFR(r.submitted_at?.slice(0, 10)) : "—"}</span>
