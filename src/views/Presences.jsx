...
import { Gauge, Plus, Trash2, Settings, Upload, Pencil } from "lucide-react";
import { ViewHeader, EmptyState, Modal } from "../lib/ui";
import { GROUPES, todayISO, formatDateFR } from "../lib/format";
import { upsertPresence, createJour, createJoursBulk, updateJour, deleteJour } from "../lib/db";
        ...
        <>
          <div className="panel">
            ...
            <select value={jourId} onChange={(e) => setJourId(e.target.value)} style={{ maxWidth: 420 }}>
              {sortedJours.map((j) => (
                <option key={j.id} value={j.id}>{formatDateFR(j.date)}{j.heure ? ` · ${j.heure}` : ""}{j.label ? ` — ${j.label}` : ""}{j.groupe ? ` (${j.groupe})` : ""}</option>
              ))}
            </select>
