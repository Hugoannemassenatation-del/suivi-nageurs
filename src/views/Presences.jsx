...
function getRow(presences, jourId, swimmerId) {
  return presences.find((p) => p.jour_id === jourId && p.swimmer_id === swimmerId) || { present: false, rpe: null };
}
...
export default function Presences({ swimmers, presences, jours, jourRpes, staffName, reload }) {
  ...
  const dayEntries = useMemo(() => presences.filter((p) => p.jour_id === jourId), [presences, jourId]);
  const presentCount = dayEntries.filter((p) => p.present).length;

  const update = async (swimmerId, patch) => {
    const prev = getRow(presences, jourId, swimmerId);
    await upsertPresence({ jour_id: jourId, date, swimmer_id: swimmerId, present: prev.present, ...patch });
    reload();
  };
  const togglePresent = (swimmerId) => {
    const p = getRow(presences, jourId, swimmerId);
        ...
        <>
          ...
          {swimmers.length === 0 ? (
            ...
            <div className="presence-grid">
              {swimmers.map((sw) => {
                const p = getRow(presences, jourId, sw.id);
