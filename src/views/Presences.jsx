...
function ManageJoursModal({ jours, onClose, onChanged }) {
  ...
  const importBulk = async () => {
    ...
    lines.forEach((line, i) => {
      ...
      const [date, groupe, label] = parts;
      if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) { ignorees.push(i + 1); return; }
      rows.push({ date, groupe: groupe || null, label: label || null });
    });
    ...
    <Modal title="Jours d'entraînement planifiés" onClose={onClose} wide>
      ...
      <div className="panel" style={{ margin: "0 0 14px" }}>
        ...
        <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>
          Une ligne par jour, séparée par des points-virgules : <code>Date (AAAA-MM-JJ);Groupe (optionnel);Label (optionnel)</code><br />
          Exemple : <code>2026-09-02;Junior Bleu;Reprise</code> ou <code>2026-09-03;;</code> pour tous les groupes sans label.
        </p>
        <textarea rows={5} value={bulkText} onChange={(e) => setBulkText(e.target.value)} placeholder={"2026-09-01;;\n2026-09-03;;\n2026-09-05;Junior Bleu;Matin"} />
