...
import { createSession, deleteSession, uploadSeancePhoto, upsertPresence, upsertSeanceRpe } from "../lib/db";

export default function Seances({ swimmers, sessions, presences, rpes, jours, readOnly, mySwimmerId, reload }) {
  ...
  return (
    <div className="view">
      ...
      {readOnly && <MonRessenti sessions={sessions} rpes={rpes} mySwimmerId={mySwimmerId} reload={reload} />}
      ...
      {sorted.length === 0 ? (
        ...
        <div className="session-list">
          {sorted.map((s) => {
            ...
            const rpeVals = rpes.filter((r) => r.session_id === s.id).map((r) => r.rpe);
              ...
              <div className="session-item" key={s.id}>
                <div>
                  ...
                  <div className="session-meta">
                    ...
                    {!readOnly && avgRpe && <><span>·</span><span>RPE moy. {avgRpe}/10 ({rpeVals.length} réponse{rpeVals.length > 1 ? "s" : ""})</span></>}
                  </div>
                </div>
                {!readOnly && (
...
function MonRessenti({ sessions, rpes, mySwimmerId, reload }) {
  const recent = [...sessions]
    .filter((s) => daysAgo(s.date) >= 0 && daysAgo(s.date) <= 5)
    .sort((a, b) => b.date.localeCompare(a.date));

  const getMine = (sessionId) => rpes.find((r) => r.session_id === sessionId && r.swimmer_id === mySwimmerId);

  const setRpe = async (sessionId, rpe) => {
    if (!mySwimmerId || rpe == null) return;
    await upsertSeanceRpe({ session_id: sessionId, swimmer_id: mySwimmerId, rpe });
    reload();
  };
  ...
  return (
    <div className="panel">
      ...
      <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>Une note par séance — 0 = repos total, 10 = effort maximal.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {recent.map((s) => {
          const mine = getMine(s.id);
          return (
            <div key={s.id} className="row-line" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span><span className="strong">{s.date}</span> — {s.titre}</span>
              <select value={mine?.rpe ?? ""} onChange={(e) => setRpe(s.id, e.target.value ? Number(e.target.value) : null)} style={{ maxWidth: 220 }}>
