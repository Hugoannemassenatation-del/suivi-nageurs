...
import { createSession, deleteSession, uploadSeancePhoto, upsertPresence, upsertJourRpe, sendNotification } from "../lib/db";
...
function ProchainsEntrainements({ jours, presences, swimmers, mySwimmerId, reload }) {
  ...
  const monNom = swimmers.find((s) => s.id === mySwimmerId)?.nom || "Un nageur";
  ...
  const declare = async (date, absent, motif) => {
    ...
    await upsertPresence({ date, swimmer_id: mySwimmerId, present: !absent, motif: finalMotif });
    if (absent) sendNotification({ type: "absence", auteur: monNom, texte: finalMotif, jourInfo: formatDateFR(date) });
    reload();
  };
