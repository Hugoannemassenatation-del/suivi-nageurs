...
export default function Seances({ swimmers, sessions, attendance, setView, setActiveSession, readOnly, reload }) {
      ...
      {sorted.length === 0 ? (
        ...
        <div className="session-list">
          {sorted.map((s) => {
            ...
            const rpeVals = rows.map((a) => a.rpe).filter((v) => v != null);
              ...
              <div className="session-item" key={s.id}>
                <div>
                  ...
                  <div className="session-meta">
                    ...
                    {!readOnly && <><span>·</span><span>{presentCount}/{swimmers.length} présents</span></>}
                    {!readOnly && avgRpe && <><span>·</span><span>RPE moy. {avgRpe}/10</span></>}
                  </div>
                </div>
                {!readOnly && (
                  <div className="session-item-actions">
                    <button className="btn-secondary" onClick={() => { setActiveSession(s.id); setView("presences"); }}>Présences</button>
