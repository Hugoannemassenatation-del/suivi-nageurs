...
import { GROUPES, BASSINS, todayISO, daysAgo, RPE_ECHELLE, formatDateFR } from "../lib/format";
...
export default function Seances({ swimmers, sessions, presences, jours, jourRpes, readOnly, mySwimmerId, reload }) {
  ...
  return (
    <div className="view">
      ...
      {readOnly && <ProchainsEntrainements jours={jours} presences={presences} swimmers={swimmers} mySwimmerId={mySwimmerId} reload={reload} />}
              ...
              <div className="session-item" key={s.id}>
                <div>
                  <div className="session-card-top">
                    ...
                    <span className="session-date">{formatDateFR(s.date)}</span>
        ...
        {recent.map((j) => {
          ...
          return (
            <div key={j.id} className="row-line" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span><span className="strong">{formatDateFR(j.date)}</span>{j.heure ? ` · ${j.heure}` : ""}{j.label ? ` — ${j.label}` : ""}</span>
...
function ProchainsEntrainements({ jours, presences, swimmers, mySwimmerId, reload }) {
  const upcoming = [...jours].filter((j) => daysAgo(j.date) <= 0).sort((a, b) => a.date.localeCompare(b.date)).slice(0, 10);

  const getMine = (jourId) => presences.find((p) => p.jour_id === jourId && p.swimmer_id === mySwimmerId);

  const declare = async (jour, absent, motif) => {
    if (!mySwimmerId) return;
    const prevMotif = getMine(jour.id)?.motif || null;
    const finalMotif = absent ? (motif !== undefined ? motif : prevMotif) : null;
    await upsertPresence({ jour_id: jour.id, date: jour.date, swimmer_id: mySwimmerId, present: !absent, motif: finalMotif });
    reload();
  };
  ...
  return (
    <div className="panel">
      ...
      <p className="login-hint" style={{ marginTop: -6, marginBottom: 10 }}>S'il y a plusieurs entraînements le même jour, tu peux te déclarer absent(e) sur un seul, ou sur les deux — coche indépendamment chaque créneau qui te concerne.</p>
          ...
          return (
            <div key={j.id} className="row-line" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span><span className="strong">{formatDateFR(j.date)}</span>{j.heure ? ` · ${j.heure}` : ""}{j.label ? ` — ${j.label}` : ""}{j.groupe ? ` (${j.groupe})` : ""}</span>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, textTransform: "none", fontWeight: 500 }}>
                  <input type="checkbox" style={{ width: "auto" }} checked={!!isAbsent} onChange={(e) => declare(j, e.target.checked)} />
                  Je serai absent(e)
                </label>
              </div>
              {isAbsent && (
                <input
                  ...
                  onBlur={(e) => declare(j, true, e.target.value)}
                  style={{ maxWidth: 320 }}
                />
              )}
            </div>
