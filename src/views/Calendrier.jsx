...
import { Plus, Trash2, Settings, ChevronLeft, ChevronRight, Pencil } from "lucide-react";
...
import {
  createEvent, updateEvent, deleteEvent, listCalendriers, createCalendrier, deleteCalendrier,
...
export default function Calendrier({ events, swimmers, readOnly, isStaff, reload }) {
  ...
  const resetForm = () => setForm({ type: TYPES_EVENT[0], nom: "", lieu: "", date_debut: todayISO(), date_fin: "", notes: "", calendrierIds: [] });

  const saveEvent = async () => {
    if (!form.nom.trim() || form.calendrierIds.length === 0) return;
    const payload = { type: form.type, nom: form.nom, lieu: form.lieu, date_debut: form.date_debut, date_fin: form.date_fin || null, notes: form.notes };
    if (editingId) {
      await updateEvent(editingId, payload);
      await setEventCalendriers(editingId, form.calendrierIds);
    } else {
  ...
  const startEditEvent = (e) => {
    setEditingId(e.id);
    const ids = eventCalendriers.filter((ec) => ec.event_id === e.id).map((ec) => ec.calendrier_id);
    setForm({ type: e.type, nom: e.nom, lieu: e.lieu || "", date_debut: e.date_debut, date_fin: e.date_fin || "", notes: e.notes || "", calendrierIds: ids });
    setOpen(true);
  };
    ...
    <div className="session-item" key={e.id}>
      ...
      {!readOnly && (
        <div className="session-item-actions">
          <button className="icon-btn" onClick={() => startEditEvent(e)}><Pencil size={15} /></button>
          <button className="icon-btn" onClick={() => remove(e.id)}><Trash2 size={15} /></button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="view">
      <ViewHeader
        ...
        action={!readOnly && (
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn-primary" onClick={() => { resetForm(); setEditingId(null); setOpen(true); }}><Plus size={16} /> Ajouter</button>
          </div>
        )}
      />
      ...
      {open && !readOnly && (
        <Modal title={editingId ? "Modifier l'événement" : "Ajouter un événement"} onClose={() => { setOpen(false); setEditingId(null); }}>
          ...
          <button className="btn-primary full" onClick={saveEvent}>{editingId ? "Enregistrer les modifications" : "Enregistrer"}</button>
        </Modal>
      )}
