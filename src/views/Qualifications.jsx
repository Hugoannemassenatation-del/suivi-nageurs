...
import { EPREUVES, BASSINS, formatSeconds, parseTimeToSeconds } from "../lib/format";
import {
...
function GrilleDetail({ grille, temps, swimmers, bestBySwimmer, onBack, reload }) {
  ...
  const addRow = async () => {
    ...
    await createGrilleTemps({
      grille_id: grille.id, epreuve: row.epreuve, bassin: row.bassin, sexe: row.sexe || null,
  ...
  const importBulk = async () => {
    ...
    lines.forEach((line, i) => {
      ...
      if (!epreuve || !bassin || secs == null) { erreurs.push(i + 1); return; }
      rows.push({
        grille_id: grille.id, epreuve, bassin, sexe: sexe || null,
  ...
  swimmers.forEach((s) => {
    matchRows(s).forEach((t) => {
      const best = bestBySwimmer[s.id + "|" + t.epreuve + "|" + t.bassin];
