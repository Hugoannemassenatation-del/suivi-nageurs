...
// ---- RPE par séance (une journée peut contenir plusieurs séances) ----
export const listSeanceRpe = () => supabase.from("seance_rpe").select("*");
export const upsertSeanceRpe = (row) =>
  supabase.from("seance_rpe").upsert(row, { onConflict: "session_id,swimmer_id" }).select().single();
