  ...
  return (
    <style>{`
      ...
      *{box-sizing:border-box;}
      ...
      .table-row.quali-row{ grid-template-columns:1.3fr 0.8fr 0.8fr 1fr 1fr 40px; }
      .table-row.quali-result-row{ grid-template-columns:1.3fr 1.1fr 0.8fr 1fr 1fr 1.2fr; }
      .table-row.jours-row{ grid-template-columns:1fr 1fr 1fr 1fr 40px; }
      .table-row.jours-row-edit{ grid-template-columns:1fr 1fr 1fr 1fr 90px; align-items:center; }
      .table-row.jours-row-edit input, .table-row.jours-row-edit select{ padding:6px 8px; font-size:12.5px; }
      ...
      .chrono-old{ text-decoration:underline; text-decoration-color:var(--ink-soft); text-underline-offset:3px; }
      ...
      .role-switch{ display:flex; gap:4px; background:var(--foam); border:1px solid var(--line-faint); border-radius:8px; padding:3px; }
      .role-switch button{ flex:1; background:none; border:none; color:var(--ink-soft); font-size:12.5px; font-weight:600; padding:8px 10px; border-radius:6px; cursor:pointer; }
      .role-switch .role-on{ background:var(--pool-mid); color:#fff; }

      .cal-nav{ display:flex; align-items:center; gap:10px; margin-bottom:12px; }
      .cal-month-label{ font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:16px; color:var(--ink); min-width:150px; text-align:center; }
      .cal-weekdays{ display:grid; grid-template-columns:repeat(7,1fr); gap:6px; margin-bottom:6px; }
      .cal-weekdays div{ text-align:center; font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color:var(--ink-soft); }
      .cal-grid{ display:grid; grid-template-columns:repeat(7,1fr); gap:6px; }
      .cal-cell{ min-height:78px; border:1px solid var(--line-faint); border-radius:8px; padding:5px 5px; background:#fff; display:flex; flex-direction:column; gap:2px; overflow:hidden; }
      .cal-cell-out{ background:var(--foam); opacity:0.55; }
      .cal-cell-today{ border-color:var(--chrono); border-width:2px; }
      .cal-cell-clickable{ cursor:pointer; }
      .cal-cell-clickable:hover{ box-shadow:0 0 0 2px var(--pool-mid) inset; }
      .cal-daynum{ font-size:11.5px; font-weight:700; color:var(--ink-soft); margin-bottom:2px; }
      .cal-chip{ font-size:10px; font-weight:600; padding:2px 5px; border-radius:5px; background:var(--pool-mid); color:#fff; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
      .cal-chip-alt{ background:var(--lane-blue); }
      .cal-chip-more{ font-size:9.5px; color:var(--ink-soft); font-weight:600; }
      .cal-chip-btn{ border:none; cursor:pointer; text-align:left; width:100%; font-family:inherit; }
      .cal-chip-selected{ background:var(--chrono) !important; box-shadow:0 0 0 2px var(--chrono); }
      ...
      /* mobile nav — masquée par défaut (desktop), affichée en dessous de 820px */
      .mobile-topbar, .mobile-bottombar, .mobile-menu-overlay{ display:none; }
      ...
      @media (max-width: 820px){
        ...
        .mobile-topbar{
          ...
          background:var(--pool-deep); color:#fff; position:sticky; top:0; z-index:20;
        }
        .mobile-topbar .brand-logo{ width:28px; height:28px; }
        .mobile-topbar-title{ flex:1; font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:14px; letter-spacing:0.03em; }
        .mobile-topbar .icon-btn{ color:#fff; }

        .view{ padding:16px 16px 84px; max-width:100%; }
        .stat-row{ grid-template-columns:repeat(2,1fr); }
        .grid-2{ grid-template-columns:1fr; }
        .form-grid-2{ grid-template-columns:1fr; }
        .session-item{ flex-direction:column; }
        .session-item-actions{ flex-direction:row; align-items:center; }

        .table-wrap-scroll, .table{ overflow-x:auto; display:block; -webkit-overflow-scrolling:touch; }
        .table-row{ min-width:560px; }

        .modal-backdrop{ padding:0; align-items:flex-end; }
        .modal{ max-width:100%; width:100%; max-height:92vh; border-radius:16px 16px 0 0; }

        .presence-check, .forme-today-tile, .btn-primary, .btn-secondary{ min-height:38px; }

        .mobile-bottombar{
          display:flex; position:fixed; bottom:0; left:0; right:0; z-index:30;
          background:#fff; border-top:1px solid var(--line-faint);
          padding:6px 4px calc(6px + env(safe-area-inset-bottom));
          box-shadow:0 -2px 10px rgba(10,31,38,0.06);
        }
        .mobile-tab{
          flex:1; display:flex; flex-direction:column; align-items:center; gap:2px;
          background:none; border:none; color:var(--ink-soft); font-size:10px; font-weight:600;
          padding:6px 2px; border-radius:8px;
        }
        .mobile-tab.active{ color:var(--chrono); }

        .mobile-menu-overlay{
          display:flex; position:fixed; inset:0; z-index:40; background:rgba(10,31,38,0.45);
        ...
        .mobile-menu-item{
          ...
          padding:12px 8px; border-radius:9px; font-size:14px; font-weight:500; color:var(--ink);
        }
        .mobile-menu-item.active{ background:var(--foam); color:var(--pool-deep); font-weight:600; }
        .mobile-menu-item .lane-count{ margin-left:auto; }
      }
    `}</style>
  );
}
