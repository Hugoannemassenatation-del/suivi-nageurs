...
import {
  ...
  MessageSquare, ShieldCheck, LogOut, ClipboardList, BarChart3,
} from "lucide-react";
...
import {
  ...
  listVma, listEvents, listMessages, listWellness, listJours, listSeanceRpe,
} from "./lib/db";
...
export default function App() {
  ...
  const reload = useCallback(async () => {
    const [sw, se, pr, rp, jo, pe, vm, ev, ms, we] = await Promise.all([
      listSwimmers(), listSessions(), listPresences(), listSeanceRpe(), listJours(), listPerformances(),
  ...
  return (
    <div className="app">
      ...
      {view === "seances" && <Seances swimmers={swimmers} sessions={sessions} presences={presences} rpes={rpes} jours={jours} readOnly={readOnly} mySwimmerId={profile.swimmer_id} reload={reload} />}
      {view === "presences" && <Presences swimmers={swimmers} presences={presences} jours={jours} reload={reload} />}
      ...
      {ficheId && (
        <FicheNageur
          ...
          sessions={sessions} presences={presences} rpes={rpes} performances={performances} vma={vma} wellness={wellness}
