...
import { LoginScreen, FirstLoginResolver } from "./lib/Auth";
import {
  listSwimmers, listSessions, listAllAttendance, listPerformances,
...
export default function App() {
  ...
  const reload = useCallback(async () => {
    const [sw, se, at, pe, vm, ev, ms, we] = await Promise.all([
      listSwimmers(), listSessions(), listAllAttendance(), listPerformances(),
      listVma(), listEvents(), listMessages(), listWellness(),
    ]);
    setSwimmers(sw.data || []);
  ...
  return (
    <div className="app">
      ...
      {view === "seances" && <Seances swimmers={swimmers} sessions={sessions} attendance={attendance} setView={setView} setActiveSession={setActiveSession} readOnly={readOnly} reload={reload} />}
      {view === "presences" && <Presences swimmers={swimmers} sessions={sessions} attendance={attendance} activeSession={activeSession} setActiveSession={setActiveSession} reload={reload} />}
      ...
      {view === "calendrier" && <Calendrier events={events} readOnly={readOnly} reload={reload} />}
      {view === "communication" && <Communication messages={messages} readOnly={readOnly} defaultAuteur={profile.nom} reload={reload} />}
      {view === "comptes" && <ComptesAdmin swimmers={swimmers} reload={reload} />}

      {ficheId && (
        <FicheNageur
          swimmer={swimmers.find((s) => s.id === ficheId)}
          sessions={sessions} attendance={attendance} performances={performances} vma={vma} wellness={wellness}
